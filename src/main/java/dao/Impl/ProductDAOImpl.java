...
public class ProductDAOImpl implements IProductDAO {
    ...
    public List<ProductDTO> getListProductDTO() {
        ...
        List<ProductDTO> productDTOList = new ArrayList<>();
        try {
            String sql = "SELECT distinct  p.productName, p.color, p.description, p.image, p.price, p.size, c.categoryID, c.categoryName " +
            ...
            for (Object[] result : results) {
                ...
                byte[] image = (byte[]) result[3];
                // byte[] image = convertBase64ToByteArray(imageString);
                double price = (double) result[4];
                int size = (int) result[5];
                int categoryID = result[6] != null ? (int) result[6] : 0;
                String categoryName = result[7] != null ? (String) result[7] : "";
                if (categoryID != 0) {
