...
@WebServlet(urlPatterns = {"/Momo_pay","/CallBack","/Momo_payTWP"})
public class MomoController extends HttpServlet {
    ...
    protected void Momo_pay1(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        HttpSession session = req.getSession();
        UserDTO userDTO = (UserDTO) session.getAttribute("user");
        ...
        if (!jsonArray.isEmpty()) {
            ...
            //Thanh Toan
            if (orderService.CreateOrderTWP(jsonArray.toString(),idUser,total,discount,feeShip)){
